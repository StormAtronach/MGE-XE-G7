...
void DistantLand::buildGrassInstanceVB(VisibleSet& grassSet) {
    batchedGrass.clear();
    ...
    HRESULT hr = vbGrassInstances->Lock(0, GrassInstStride * grassSet.Size(), (void**)&vbwrite, D3DLOCK_DISCARD);
...
void DistantLand::renderGrassInstZ() {
    ...
    depthReplayDips += batchedGrass.size();
}
...
void DistantLand::renderGrassCommon(ID3DXEffect* e) {
    ...
    for (const auto& grass : batchedGrass) {
        ...
        device->SetStreamSource(0, grass.first->vBuffer, 0, SIZEOFGRASSVERT);
