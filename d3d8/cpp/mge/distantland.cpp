...
namespace {
    ...
    bool isFinite(const D3DVECTOR& value) {
        return std::isfinite(value.x) && std::isfinite(value.y) && std::isfinite(value.z);
    }

    bool isFinite(const D3DCOLORVALUE& value) {
        return std::isfinite(value.r) && std::isfinite(value.g) &&
            std::isfinite(value.b) && std::isfinite(value.a);
    }

    bool hasMatchingSignature(
        const DistantLand::ObservedPointLight& observation,
        const LightState::Light& light) {
        return std::memcmp(&observation.diffuse, &light.diffuse, sizeof(light.diffuse)) == 0 &&
            std::memcmp(&observation.falloff, &light.falloff, sizeof(light.falloff)) == 0;
    }

    int compareSignatures(
        const DistantLand::ObservedPointLight& lhs,
        const DistantLand::ObservedPointLight& rhs) {
        int result = std::memcmp(&lhs.diffuse, &rhs.diffuse, sizeof(lhs.diffuse));
        return result != 0
            ? result
            : std::memcmp(&lhs.falloff, &rhs.falloff, sizeof(lhs.falloff));
    }

    bool derivePostLightRadius(const D3DVECTOR& attenuation, float& radius) {
        if (!isFinite(attenuation) || attenuation.x < 0 || attenuation.y < 0 || attenuation.z < 0) {
            return false;
        }

        const double target = 1.0 / kPostLightCutoff;
        const double remaining = target - attenuation.x;
        if (remaining <= 0) {
            return false;
        }
        ...
        if (attenuation.z > 0) {
            const double discriminant =
                attenuation.y * attenuation.y + 4.0 * attenuation.z * remaining;
            if (!std::isfinite(discriminant) || discriminant < 0) {
                return false;
            }

            const double denominator = attenuation.y + std::sqrt(discriminant);
            if (denominator <= 0) {
                return false;
            }
            result = 2.0 * remaining / denominator;
        } else if (attenuation.y > 0) {
            result = remaining / attenuation.y;
        } else {
            return false;
        }
        ...
        radius = static_cast<float>(std::min(result, 0.5 * DistantLand::kCellSize));
        return std::isfinite(radius) && radius > 0;
    }
}

void DistantLand::onResolveDuringInit() {
    // The save/new-game world data has resolved. Finish any outstanding upload
    // work before returning to the engine's load path.
    worldResolved = true;

    if (state == InitState::DeviceResourcesReady) {
        if (pumpActive && !uploadComplete) {
            drainUploadPump();
        }
        finalizeUploadIfReady();
    } else if (state == InitState::RenderReady) {
...
void DistantLand::beginDepthFrame() {
    stage1UsedNativeDepth = false;
    nativeStage2Eligible = false;
    observedPostLights.clear();
    postLightCount = 0;
    postPointLightsRequested =
        (Configuration.MGEFlags & USE_HW_SHADER) && PostShaders::chainWantsPointLights();
}

void DistantLand::observePostLights(const RenderedState& rs, const LightState& lightrs) {
    if (!postPointLightsRequested || !rs.useLighting) {
        return;
    }

    for (DWORD id : lightrs.active) {
        auto found = lightrs.lights.find(id);
        if (found == lightrs.lights.end()) {
            continue;
        }

        const LightState::Light& light = found->second;
        if (light.type != D3DLIGHT_POINT ||
            !isFinite(light.position) || !isFinite(light.diffuse) || !isFinite(light.falloff)) {
            continue;
        }

        bool matched = false;
        for (auto& observation : observedPostLights) {
            if (observation.id == id && hasMatchingSignature(observation, light)) {
                observation.position = light.position;
                matched = true;
                break;
            }
        }
        ...
        if (!matched) {
            observedPostLights.push_back({ id, light.position, light.diffuse, light.falloff });
        }
    }
}

void DistantLand::finalizePostLights() {
    struct Candidate {
        const ObservedPointLight* observation;
    ...
    candidates.reserve(observedPostLights.size());

    const bool hasFiniteEye =
        std::isfinite(eyePos.x) && std::isfinite(eyePos.y) && std::isfinite(eyePos.z);

    for (const auto& observation : observedPostLights) {
        float sharedFalloffConstant = 0.33f;
        DecodedPointLight decoded = decodeMorrowindPointLight(
            observation.diffuse,
            observation.falloff,
            sharedFalloffConstant);

        if (!isFinite(decoded.diffuse) ||
            (decoded.diffuse.r <= 0 && decoded.diffuse.g <= 0 && decoded.diffuse.b <= 0)) {
            continue;
        }

        float radius;
        if (!derivePostLightRadius(decoded.attenuation, radius)) {
            continue;
        }

        double score = 0;
        if (hasFiniteEye) {
            const double dx = static_cast<double>(observation.position.x) - eyePos.x;
            const double dy = static_cast<double>(observation.position.y) - eyePos.y;
            const double dz = static_cast<double>(observation.position.z) - eyePos.z;
            score = std::max(0.0, std::sqrt(dx * dx + dy * dy + dz * dz) - radius);
        }

        candidates.push_back({ &observation, decoded, radius, score });
    }

    if (candidates.size() > kMaxPostPointLights) {
        std::partial_sort(
            candidates.begin(),
            candidates.begin() + kMaxPostPointLights,
            candidates.end(),
            [](const Candidate& lhs, const Candidate& rhs) {
                if (lhs.score != rhs.score) {
                    return lhs.score < rhs.score;
                }
                if (lhs.observation->id != rhs.observation->id) {
                    return lhs.observation->id < rhs.observation->id;
                }
                return compareSignatures(*lhs.observation, *rhs.observation) < 0;
            });
        candidates.resize(kMaxPostPointLights);
    }

    for (const auto& candidate : candidates) {
        const auto& position = candidate.observation->position;
        postLightPositions[postLightCount] =
            D3DXVECTOR4(position.x, position.y, position.z, candidate.radius);
        postLightColours[postLightCount] = D3DXVECTOR4(
            candidate.decoded.diffuse.r,
            candidate.decoded.diffuse.g,
            candidate.decoded.diffuse.b,
            0.0f);
        ++postLightCount;
    }
}

IDirect3DSurface9* DistantLand::nativeDepthSourceSurface() {
    switch (nativeDepthBackend) {
        case NativeDepthBackend::IntzMainDsv:
            return surfDepthStencil;
        case NativeDepthBackend::DxvkMsaaResolve:
            return surfAutoDepthStencil;
        default:
            return nullptr;
    }
}

void DistantLand::depthBufferCleared() {
    IDirect3DSurface9* sourceDepthStencil = nativeDepthSourceSurface();
    if (!sourceDepthStencil) {
        return;
    }

    IDirect3DSurface9* activeDepthStencil = nullptr;
    if (SUCCEEDED(device->GetDepthStencilSurface(&activeDepthStencil))) {
        if (activeDepthStencil == sourceDepthStencil) {
            dsvMayBeNoncanonical = false;
        }
        activeDepthStencil->Release();
    }
}

bool DistantLand::projectionIsCanonical(const D3DXMATRIX& projection) {
    D3DXMATRIX canonical = projection;
    editProjectionZ(&canonical, kDistantNearPlane, Configuration.DL.DrawDist * kCellSize);
    ...
    return nearlyEqual(projection._33, canonical._33)
        && nearlyEqual(projection._43, canonical._43);
}

void DistantLand::trackDepthProjection(const D3DMATRIX* projection) {
    if (nativeDepthBackend != NativeDepthBackend::None
        && projection
        && !projectionIsCanonical(*projection)) {
...
void DistantLand::renderStage0() {
    ...
    if (!isRenderCached) {
        ...
        if (isDistantCell()) {
            ...
            // Close the render frame for the adaptive horizon gate while horizon culling is enabled.
            // Keep this independent of cullDistantStatics' sortVisibleSet so a frame where only the
            // water reflection issues a precise static query (e.g. distant statics disabled but
            // reflected statics enabled) still ticks the gate, and stats from this frame's main
            // and reflected static passes are never carried into the next frame.
            if (Configuration.Horizon.Culling) {
                ipcClient.finishHorizonFrame();
            }
...
void DistantLand::renderStage1() {
    ...
    if (!isRenderCached) {
        ...
        if (isDistantCell() && staticsUploaded) {
        ...
        // Depth texture from the main INTZ DSV, with geometry replay fallback.
        effectDepth->Begin(&passes, D3DXFX_DONOTSAVESTATE);
        const bool captured =
            Configuration.EnableNativeDepthCapture
            && nativeDepthBackend != NativeDepthBackend::None
            && !dsvMayBeNoncanonical
            && projectionIsCanonical(mwProj)
            && SUCCEEDED(captureNativeDepth(NativeDepthMode::Replace, mwProj));
        if (captured) {
        ...
        stage1UsedNativeDepth = captured;
        nativeStage2Eligible = captured;
        effectDepth->End();
...
void DistantLand::renderStage2() {
    ...
    if (!isRenderCached) {
        ...
        // Merge later-scene depth when it is comparable with stage 1.
        effectDepth->Begin(&passes, D3DXFX_DONOTSAVESTATE);
        D3DXMATRIX activeProjection;
        const bool haveProjection =
            SUCCEEDED(device->GetTransform(D3DTS_PROJECTION, &activeProjection));
        const bool captured =
            Configuration.EnableNativeDepthCapture
            && stage1UsedNativeDepth
            && nativeStage2Eligible
            && nativeDepthBackend != NativeDepthBackend::None
            && !dsvMayBeNoncanonical
            && haveProjection
            && projectionIsCanonical(activeProjection)
            && SUCCEEDED(captureNativeDepth(NativeDepthMode::MergeNearest, activeProjection));
        if (captured) {
...
void DistantLand::updatePostShader(MGEShader* shader) {
    ...
    shader->SetInt(EV_pointlightcount, postLightCount);
    if (postLightCount > 0) {
        shader->SetFloatArray(
            EV_pointlightpos,
            reinterpret_cast<const float*>(postLightPositions),
            4 * postLightCount);
        shader->SetFloatArray(
            EV_pointlightcol,
            reinterpret_cast<const float*>(postLightColours),
            4 * postLightCount);
    }
...
void DistantLand::scanDynamicVisGroups() {
    ...
    std::uint16_t i = 0;
    for (auto& vis : dynamicVisGroups) {
        // The switch below has no default case; an unrecognised DataSource
        // would otherwise leave this undefined before the range test.
        int value = 0;
        auto groupIndex = i++;
        ...
        case DynamicVisGroup::DataSource::UniqueObject: {
            ...
            const int deletedRecordFlag = 0x20;
            value = (mwBridge->getRecordFlags(vis.gameObject) & (disabledRecordFlag | deletedRecordFlag)) == 0;
            break;
        }
        }
...
bool DistantLand::inspectIndexedPrimitive(int sceneCount, const RenderedState* rs, const FragmentState* frs, LightState* lightrs) {
    ...
    if (rs->zWrite && !isLandSplat && !isDecal) {
        ...
        auto& recorded = recordMW.back();

        const bool indexedSkinning =
            rs->vertexBlendState != D3DVBF_DISABLE &&
            (rs->fvf & D3DFVF_LASTBETA_UBYTE4) != 0;
        if (indexedSkinning) {
            recorded.skinPaletteOffset = static_cast<std::uint32_t>(recordedSkinPalettes.size());
            recorded.skinPaletteCount = MGE_INDEXED_SKINNING_PALETTE_SIZE;
            const auto palette = FixedFunctionShader::getSkinningWorldViewTransforms();
            recordedSkinPalettes.insert(
                recordedSkinPalettes.end(),
                palette,
                palette + MGE_INDEXED_SKINNING_PALETTE_SIZE
            );
        }
...
DistantLand::RecordedState::RecordedState(const RenderedState& state)
    : RenderedState(state), skinPaletteOffset(0), skinPaletteCount(0) {
    vb->AddRef();
    ib->AddRef();
    if (texture) {
        texture->AddRef();
...
DistantLand::RecordedState::RecordedState(RecordedState&& source) noexcept
    : RenderedState(source),
      skinPaletteOffset(source.skinPaletteOffset),
      skinPaletteCount(source.skinPaletteCount) {
    source.vb = nullptr;
    source.ib = nullptr;
