...
struct ConfigurationStruct {
    ...
    bool AutomaticDistantLandRebuild;
    bool EnableNativeDepthCapture;
    bool EnableNativePplPackets;
    // Restart-required: the engine patch it authorizes cannot be undone for
    // the lifetime of the process.
    bool ExpandedLightLimit;
    ...
    // Terrain horizon culling. Applied host-side and tunable live from the MWSE menu via FFI
    // (see apiffi.cpp). Kept outside DL because DL is reinterpret_cast to the MWSE-shared
    // DistantLandRenderConfig (api.h) and must stay layout-identical to it. The INI keys still
    // live in the "Distant Land" section (see inidata.h).
    struct {
        bool Culling;
        float BiasZ;
        float ObjectBiasZ;
